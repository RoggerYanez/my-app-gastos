...
export default function HerramientasScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: isDark ? '#121212' : '#f5f5f5' }]}>
      <Text style={[styles.title, { color: isDark ? '#fff' : '#000' }]}>Herramientas</Text>
      
      <View style={[styles.card, { backgroundColor: isDark ? '#1e1e1e' : '#fff' }]}>
        <Text style={[styles.label, { color: isDark ? '#fff' : '#000' }]}>Exportar Gastos</Text>
        <Text style={[styles.desc, { color: isDark ? '#aaa' : '#666' }]}>
          Descarga un archivo con todo tu historial de gastos.
        </Text>
